...
app.NoteListDatabase.TABLE = "notes"
...
function app.NoteListDatabase.Note:setNote(value)
	self._note = value
	do return self end
end

function app.NoteListDatabase.Note:getNote()
...
function app.NoteListDatabase:forContext(ctx)
	local cstr = _g.jk.env.EnvironmentVariable:get("NOTES_DATABASE")
...
function app.NoteListDatabase:updateTables()
	local tbl = _g.jk.sql.SQLTableInfo:forName(_g.app.NoteListDatabase.TABLE)
	do tbl:addStringKeyColumn("id") end
	do tbl:addStringColumn("note") end
	do self:updateTable(tbl) end
end

function app.NoteListDatabase:addValue(tbl)
	if not (tbl ~= nil) then
		do return nil end
	end
	do tbl:setId("1") end
	do tbl:setTimeStampAddedValue(_g.jk.time.SystemClock:asUTCSeconds()) end
	if not self.db:executeSync(self.db:prepareInsertStatementSync(_g.app.NoteListDatabase.TABLE, tbl:toDynamicMap())) then
		do return nil end
	end
	do return tbl end
end

function app.NoteListDatabase:updateValue(id, tbl)
	if not (tbl ~= nil) then
		do return false end
	end
	do tbl:setTimeStampLastUpdatedValue(_g.jk.time.SystemClock:asUTCSeconds()) end
	do
		local criteria = _g.app.NoteListDatabase.Note._construct0(_g.app.NoteListDatabase.Note._create())
		do criteria:setId(id) end
		do return self.db:executeSync(self.db:prepareUpdateStatementSync(_g.app.NoteListDatabase.TABLE, criteria:toDynamicMap(), tbl:toDynamicMap())) end
	end
end

function app.NoteListDatabase:deleteValue(id)
	local criteria = _g.app.NoteListDatabase.Note._construct0(_g.app.NoteListDatabase.Note._create())
	do criteria:setId(id) end
	do return self.db:executeSync(self.db:prepareDeleteStatementSync(_g.app.NoteListDatabase.TABLE, criteria:toDynamicMap())) end
end

function app.NoteListDatabase:getValues()
	local v = {}
	local it = self.db:querySync(self.db:prepareQueryAllStatementSync(_g.app.NoteListDatabase.TABLE, nil, nil))
	if not (it ~= nil) then
		do return nil end
	end
	do return nil end
